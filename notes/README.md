...
| Record | What it settles |
|---|---|
| `2026-08-22-decision-the-seat-name-edit.md` | 필수 단일 필드는 `PATCH`가 아니라 `PUT`이다; 빈 자리는 미리 채울 수 없고 주소 자체가 없다; 숫자가 움직일 수 없는 뮤테이션은 집계를 싣지 않고 `wedding/`에 남는다; 이름 규칙은 `@SeatName` 하나 |
